...
`tools/list` at `https://gateway.pipeworx.io/dailymed/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_drugs \
  -H 'Content-Type: application/json' \
  -d '{"name":"Aspirin"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_drugs`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
